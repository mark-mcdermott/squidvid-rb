...
class TestSquidvid < Test::Unit::TestCase
  ...
  def test_get_songs_str
    ...
    assert_equal(song_str_expected,song_str_actual,"song_str_expected string is incorrect - is #{song_str_actual}")
  end

  def test_images_str
    temp_folder="temp"
    images=['10k-ty-beat.png','1fordibiase.png']
    num_songs=2
    images_str_expected = '-i temp/10k-ty-beat.png -i temp/1fordibiase.png '
    images_str_actual = Squidvid.new().get_images_str(temp_folder,num_songs,images)
    assert_not_nil(images_str_actual, "images_str_actual is nil" )
    assert_kind_of(String,images_str_actual, "images_str_actual is not a string" )
    assert_equal(images_str_expected,images_str_actual,"images_str_actual string is incorrect - is #{images_str_actual}")
  end

  def test_get_filter_str
    num_songs = 2
    start_points = [0,146.7820]
    end_points = [146.7820,269.6359]
    temp_folder = 'temp'
    temp_song_text_base_filename = 'tempSongTextFile'
    album_art_coordinates = 'W*0.036:H*0.59'
    font_filepath = '/Library/Fonts/Helvetica-Bold.ttf'
    font_color = 'white'
    font_size = 120
    text_coordinates = 'x=w*.035:y=h*.95-text_h'
    text_line_spacing = 25
    filter_string_expected="-filter_complex [1:0][2:0]concat=n=2:v=0:a=1[aud],[0:v][3:v]overlay=W*0.036:H*0.59:enable='between(t,0.0000,146.7820)'[temp0],[temp0]drawtext=fontfile=/Library/Fonts/Helvetica-Bold.ttf:fontcolor=white:fontsize=120:x=w*.035:y=h*.95-text_h:line_spacing=25:textfile=temp/tempSongTextFile-1.txt:enable='between(t,0.0000,146.7820)'[temp1],[temp1][4:v]overlay=W*0.036:H*0.59:enable='between(t,146.7820,269.6359)'[temp2],[temp2]drawtext=fontfile=/Library/Fonts/Helvetica-Bold.ttf:fontcolor=white:fontsize=120:x=w*.035:y=h*.95-text_h:line_spacing=25:textfile=temp/tempSongTextFile-2.txt:enable='between(t,146.7820,269.6359)'"
    filter_string_actual=Squidvid.new().get_filter_str(num_songs,start_points,end_points,temp_folder,temp_song_text_base_filename,album_art_coordinates,
                                                       font_filepath,font_color,font_size,text_coordinates,text_line_spacing)
    assert_equal(filter_string_expected,filter_string_actual,"filter_string_actual string is incorrect - is #{filter_string_actual}")
  end

  def test_get_output_file_str
    quality = 'ultrafast'
    output_path_and_filename = 'output/stream-5-mins-0627210303.mp4'
    temp_folder = 'temp'
    output_str_expected = " -map [aud] -preset ultrafast -y -loglevel quiet output/stream-5-mins-0627210303.mp4 -progress temp/ffmpeg-progress.log"
    output_str_actual = Squidvid.new().get_output_file_str(quality, output_path_and_filename, temp_folder)
    assert_equal(output_str_expected,output_str_actual,"output_str_actual string is incorrect - is #{output_str_actual}")
  end

  def test_get_num_frames_total
    output_filepath = 'examples/example-output.mp4'
    num_frames_expected = 421
    num_frames_actual = Squidvid.new().get_num_frames_total(output_filepath)
    assert_equal(num_frames_expected,num_frames_actual,"num_frames_actual string is incorrect - is #{num_frames_actual}")
  end
  ...
  def test_ffprobe_frames_call
    output_filepath = 'examples/example-output.mp4'
    num_frames_expected = 421
    num_frames_actual = Squidvid.new().ffprobe_frame_count(output_filepath)
    assert_equal(num_frames_expected,num_frames_actual,"num_frames_actual string is incorrect - is #{num_frames_actual}")
  end
